...
def render():
    st.markdown("""
    <h1 style='text-align:center;background:linear-gradient(135deg,#f093fb,#f5576c);
    -webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:2.4rem;'>
    📝 String Matching Visualizer</h1>
    ...
    with tab_viz:
        ...
        if st.button("▶️ Auto-Play", key="str_play"):
    ...
    with tab_compare:
        ...
        <div style='background:rgba(26,31,46,0.6);border-radius:12px;padding:20px;border:1px solid rgba(255,255,255,0.08);margin-bottom:1rem;'>
            ...
            <p style='color:#94a3b8;font-size:0.9rem;'>Test algorithms on a much larger custom text block. (Keep under 2000 chars to avoid memory issues from tracking all steps).</p>
        </div>
        """, unsafe_allow_html=True)
