...
from modules.sorting.sorting_visualizer import build_sort_figure
...
from utils.constants import SORTING_ALGORITHMS, COLOR_COMPARE, COLOR_ACTIVE, COLOR_SORTED, COLOR_DEFAULT
from utils.ui_components import page_header, control_panel, error_boundary, legend_bar, recommendation_box
...
@error_boundary("Sorting")
def render():
    page_header(
        "Sorting Algorithm Visualizer",
        "Compare Selection, Insertion, Merge, and Quick Sort side-by-side.",
        "sorting",
    )
    ...
    with tab_viz:
        ...
        st.plotly_chart(fig, use_container_width=True, key="sort_viz_chart")

        if explain_mode:
            st.info(f"**Step {step_num}:** {step['description']}")

        # Auto-play button
        if st.button("Auto-Play (0.4s/step)", key="sort_autoplay"):
    ...
    with tab_compare:
        with control_panel("Benchmark Configuration"):
            st.markdown(
                ...
                "Configure a larger dataset strictly for performance comparison. "
                "(Max 500 to avoid memory overflow from step tracking)</p>",
        ...
        if "sort_bench_results" in st.session_state:
            ...
            c1, c2 = st.columns(2)
            c1.plotly_chart(timing_bar_chart(t_map, "Execution Time"), use_container_width=True, key="sort_timing_chart")
            c2.plotly_chart(operations_bar_chart(op_map, "Comparisons Made"), use_container_width=True, key="sort_ops_chart")
    ...
    with tab_complexity:
        st.markdown("### Time and Space Complexity Reference")
